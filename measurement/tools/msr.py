...
def main():
    parser = argparse.ArgumentParser(
        description="Display Intel RAPL package and DRAM power in watts."
    )
    ...
    package_history = collections.deque(maxlen=max(1, SMOOTH_SAMPLES))
    dram_history = collections.deque(maxlen=max(1, SMOOTH_SAMPLES))
    ...
    try:
        while True:
            ...
            package_w = (package_delta_uj / 1_000_000) / elapsed_s
            dram_w = (dram_delta_uj / 1_000_000) / elapsed_s
            ...
            displayed_total_w = displayed_package_w + displayed_dram_w
            ...
            if sample_ns > next_sample_ns:
                missed = (sample_ns - next_sample_ns) // interval_ns + 1
